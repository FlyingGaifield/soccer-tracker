function [positions, rect_results, time] = tracker(video_path, img_files, pos, target_sz, ...
	...
	features, show_visualization,homography)
...
% 	window_sz = 2 .^ nextpow2(window_sz);
    ...
    temp = gaussian_shaped_labels(output_sigma, floor(window_sz / cell_size));
...
% 		end
		...
		if frame > 1
            ...
            for i=1:size(search_size,2)
                ...
                %subplot(2,1,2);
                %imshow(patch);
                jj = get_features(patch, features, cell_size, cos_window,w2c);
                zf = fft2(jj);
            ...
            test_para = (response == max(response(:))) ;
            [y,x]  = find(test_para, 1);
			[vert_delta, tmp,horiz_delta] = find(test_para, 1);
            szid = floor(tmp/(size(cos_window,2)+1)); %因为序号是连续下去的
...
% 		patch = get_subwindow(im, pos, window_sz);
	    ...
	    if show_visualization,
                ...
                figure(3)
                hold on 
                plot(box(1)+box(3)/2,box(2)+box(4),'r.')
                figure(4)
                y = homography_transform([(box(2)+box(4)),(box(1)+box(3)/2)]', homography)
                hold on 
